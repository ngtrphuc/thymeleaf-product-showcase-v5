...
function AvailabilityBadge({ product }: { product: ProductSummary }) {
  return (
    ...
    >
      {product.availabilityLabel || "Chưa có sẵn"}
    </span>
  );
}
    ...
    <main className="space-y-6">
      ...
      <section className="glass-panel grid gap-6 rounded-3xl p-5 md:grid-cols-2 md:p-8">
        ...
        <div className="space-y-4">
          ...
          <p className="text-3xl font-bold text-[var(--color-primary-strong)]">
          ...
          <p className="text-sm text-slate-600">
            Trả góp dự kiến từ{" "}
            <strong>{formatPriceVnd(product.monthlyInstallmentAmount)}</strong> / tháng.
          </p>
        </div>
      </section>

      <section className="space-y-3">
        <h2 className="text-xl font-bold text-slate-900">Sản phẩm gợi ý</h2>
        {detail.recommendedProducts.length === 0 ? (
          <div className="glass-panel rounded-2xl p-5 text-sm text-slate-600">
            Chưa có sản phẩm gợi ý ở thời điểm hiện tại.
          </div>
        ) : (
